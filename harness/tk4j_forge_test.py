"""TK4J Forge compat test: real Forge server + RCON kill harness.
Usage: python tk4j_forge_test.py <mc_version> <forge_version> <jar_path>
"""
import json, os, shutil, socket, struct, subprocess, sys, time, urllib.request

VER, FORGE, JAR = sys.argv[1], sys.argv[2], sys.argv[3]
ROOT = os.path.dirname(os.path.abspath(__file__))
DIR = os.path.join(ROOT, f'forge-{VER}')
...
argsfile = os.path.join(DIR, 'libraries', 'net', 'minecraftforge', 'forge', f'{VER}-{FORGE}', 'win_args.txt')
if not os.path.exists(argsfile):
    inst = os.path.join(DIR, 'forge-installer.jar')
    if not os.path.exists(inst):
        url = f'https://maven.minecraftforge.net/net/minecraftforge/forge/{VER}-{FORGE}/forge-{VER}-{FORGE}-installer.jar'
    ...
    if not os.path.exists(argsfile):
        log('INSTALL FAILED'); log(r.stdout[-800:]); sys.exit(1)
    log('installed')
...
log(f'forge server starting pid={proc.pid} ({VER}-{FORGE})')
...
log(f'=== FORGE {VER} RESULTS ===')
for k, v in results.items(): log(f'  {k}: {"PASS" if v else "FAIL"}')
log(f'=== FORGE {VER}: {"ALL PASS" if ok else "FAILURES PRESENT"} ===')
